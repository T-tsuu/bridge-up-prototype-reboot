...
const Auth = {
    ...
    logout() {
        this.clear();
        ProfileStore.clear();
        // UserStore and ProfileVault are intentionally NOT cleared — required for re-login.
    ...
    requireAuth(allowedRoles=[]) {
        ...
        if (allowedRoles.length && !allowedRoles.includes(s.role)) {
            window.location.href = s.role === "student"    ? "student-dashboard.html"
                                 : s.role === "recruiter"  ? "recruiter-dashboard.html"
...
const ProfileStore = {
    ...
    save(data) {
        ...
        // Mirror into vault so profile survives logout
    ...
    patch(patch) {
        ...
        this.save(updated); // also updates vault via save()
    },
    // Restore from vault by id (used on re-login after logout)
...
function resolveProfile(session) {
    ...
    // 2. Vault restore (user logged back in after logout)
    const vaulted = ProfileVault.get(session.id);
    if (vaulted) {
        ProfileStore.save(vaulted); // restore to active slot (also re-saves to vault, idempotent)
        return vaulted;
    }

    // 3. BridgeDB demo accounts
    ...
    // 4. Stale stored profile (edge case: id mismatch but something is there)
...
function renderJobCard(job, opts={}) {
    ...
    <div class="job-card" onclick="window.location.href='job-listing.html?id=${job.id}'">
        <div class="job-card-header">
            ...
            <span class="badge ${
                job.type === 'Internship'        ? 'badge-blue'   :
                job.type === 'Freelance Project' ? 'badge-orange' : 'badge-gray'
            }">${job.type}</span>
        </div>
...
function buildNavbar(activePage="", role="") {
    ...
    if (!session) return "";
    const dashLink = role === "student"    ? "student-dashboard.html"
...
function initStickyWidget() {
    ...
    widget.innerHTML = `
        <div class="sticky-widget__icon"><span>📋</span></div>
        <div class="sticky-widget__content">
            <p>Interested in Bridge Up?</p>
            <a href="https://forms.gle/Ga3BMMrb7mSNCXHE9" target="_blank" class="sticky-widget__btn">
                Give Feedback →
            </a>
        </div>
    `;
    ...
    setTimeout(() => { triggerWiggle(); setInterval(triggerWiggle, 5000); }, 3000);
}
